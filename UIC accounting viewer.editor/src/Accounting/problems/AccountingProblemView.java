...
public class AccountingProblemView extends ViewPart {
				...
				if(column == 0){
					...
					if (diagnostic.getData()!= null && !diagnostic.getData().isEmpty()) {
						if (diagnostic.getData().get(0) instanceof EObject) {
							return AccountingUtils.getLabelText((EObject)diagnostic.getData().get(0));
						}
					}
				}
			}
	...
	public static AccountingProblemView getInstance() {
		if (me == null) {
			me = new AccountingProblemView();
		}
		return me;
	}
