...
class CodeListInitializer {
	
	public static void createGenericReductionCards(FareStructure fareStructure, AccountingViewerData tool) {
		createReductionCard(fareStructure,GenericReductionCards.UIC_EURAIL_1.getName(),"Eurail Pass 1st Class", AccountingUtils.findCarrier(tool,"9902")); //$NON-NLS-1$ //$NON-NLS-2$
		createReductionCard(fareStructure,GenericReductionCards.UIC_INTERRAIL_1.getName(),"Interrail Pass 1st Class", AccountingUtils.findCarrier(tool,"9902"));		 //$NON-NLS-1$ //$NON-NLS-2$
		...
		createReductionCard(fareStructure,GenericReductionCards.UIC_EURAIL_2.getName(),"Eurail Pass 2nd Class", AccountingUtils.findCarrier(tool,"9902")); //$NON-NLS-1$ //$NON-NLS-2$
		createReductionCard(fareStructure,GenericReductionCards.UIC_INTERRAIL_2.getName(),"Interrail Pass 2nd Class", AccountingUtils.findCarrier(tool,"9902"));		 //$NON-NLS-1$ //$NON-NLS-2$
		...
		createReductionCard(fareStructure,GenericReductionCards.UIC_EURAIL.getName(),"Eurail Pass - deprecated-use class specific cards", AccountingUtils.findCarrier(tool,"9902")); //$NON-NLS-1$ //$NON-NLS-2$
		createReductionCard(fareStructure,GenericReductionCards.UIC_INTERRAIL.getName(),"Interrail Pass - deprecated-use class specific cards", AccountingUtils.findCarrier(tool,"9902"));		 //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static ReductionCard createReductionCard(FareStructure fareStructure, String id, String name, Carrier carrier) {
		...
		Text text =  AccountingFactory.eINSTANCE.createText();
		ReductionCard card =  AccountingFactory.eINSTANCE.createReductionCard();
		...
		if (fareStructure.getTexts() == null) {
			fareStructure.setTexts(AccountingFactory.eINSTANCE.createTexts());
		}
		fareStructure.getTexts().getTexts().add(text);
		if (fareStructure.getReductionCards() == null) {
			fareStructure.setReductionCards(AccountingFactory.eINSTANCE.createReductionCards());
		}
	...
	public Command createPopulateCountriesCommand(EditingDomain domain, CodeLists codeLists) {
		...
		Countries countries = AccountingFactory.eINSTANCE.createCountries();
		populateUICcountries(countries);
		
		SetCommand command = new SetCommand(domain, countries, AccountingPackage.Literals.CODE_LISTS__COUNTRIES, countries);
	...
	public static Language createLanguage(String code, String name) {
		Language language = AccountingFactory.eINSTANCE.createLanguage();
	...
	public static Country createCountry(int code, String name, String ISOcodeA2, CharacterSet charSet) {		
		Country country = AccountingFactory.eINSTANCE.createCountry();
	...
	public static void populateCurrencies(Currencies currencies) {
		...
		addCurrency("EUR", "Euro", currencies);
		addCurrency("PLN","Zloty", currencies);
		addCurrency("BGN","Lev", currencies);
		addCurrency("HRK","Kuna", currencies);
		addCurrency("RON","Leu", currencies);
		addCurrency("CZK","Koruna", currencies);
		addCurrency("HUF","Forint", currencies);
	    addCurrency("SEK","Krona", currencies);
	    addCurrency("DKK","Krone", currencies);
		addCurrency("CHF","Frank", currencies);
		addCurrency("ISK","Krona", currencies);
		addCurrency("NOK","Krone", currencies);
		addCurrency("ALL","Lek", currencies);
		addCurrency("RSD","Dinar", currencies);
		addCurrency("MKD","Denar", currencies);
	    addCurrency("TRY","Lira", currencies);	 	 
		addCurrency("BAM", "Convertible Mark", currencies);
		addCurrency("GBP", "Pound Sterling", currencies);
		
	}
	
	public static void addCurrency(String isoCode, String name, Currencies currencies) {
		
		if (currencies == null) return;
		
		Currency currency = AccountingFactory.eINSTANCE.createCurrency();
		currency.setIsoCode(isoCode); //$NON-NLS-1$
